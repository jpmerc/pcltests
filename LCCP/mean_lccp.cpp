...
void printSuperVoxels(std::map<uint32_t, pcl::Supervoxel<pcl::PointXYZRGBA>::Ptr> *voxels, std::multimap<uint32_t, uint32_t> *adjacency, boost::shared_ptr<pcl::visualization::PCLVisualizer> visual, bool showGraph);
...
void superVoxels_clustering(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud){
    ...
    pcl::SupervoxelClustering<PointT> super (SUPERVOXEL_VOXEL_RESOLUTION, SUPERVOXEL_SEED_RESOLUTION);
    ...
    // Voxel Cloud Colors
    PointLCloudT::Ptr colored_cloud = super.getLabeledCloud();
   ...
   // cout << "Total Number of points : " << size << endl;
    ...
    std::map <uint32_t, pcl::Supervoxel<PointT>::Ptr > lccp_clusters;
    std::multimap<uint32_t, uint32_t> lccp_adjacency;

    pcl::LCCPSegmentation<PointT> lccp;
    lccp.setConcavityToleranceThreshold (concavity_tolerance_threshold);
    lccp.setSanityCheck (false);
    lccp.setSmoothnessCheck (true, SUPERVOXEL_VOXEL_RESOLUTION, SUPERVOXEL_SEED_RESOLUTION, smoothness_threshold);
    lccp.setKFactor (0);
    lccp.segment (lccp_clusters, lccp_adjacency);




    printSuperVoxels(&lccp_clusters, &lccp_adjacency, pclViewer5, true);
...
void printSuperVoxels(std::map<uint32_t, pcl::Supervoxel<pcl::PointXYZRGBA>::Ptr> *voxels, std::multimap<uint32_t, uint32_t> *adjacency, boost::shared_ptr<pcl::visualization::PCLVisualizer> visual, bool showGraph){
    ...
    int i = 0;
    int size = 0;
    for(std::map<uint32_t, pcl::Supervoxel<pcl::PointXYZRGBA>::Ptr >::iterator it = voxels->begin(); it != voxels->end(); ++it){
        pcl::PointCloud<pcl::PointXYZRGBA>::Ptr pc = it->second->voxels_;
        cout << "PC Size (" << it->first << ") : " << pc->size() << endl;
        size += pc->size();
        pcl::visualization::PointCloudColorHandlerRandom<pcl::PointXYZRGBA> randColor(pc);
    ...
    // Print Graph Connectivity
    if(showGraph){
        std::multimap<uint32_t,uint32_t>::iterator label_itr = adjacency->begin ();
        for ( ; label_itr != adjacency->end (); )
        {
            //First get the label
            int supervoxel_label = label_itr->first;
            ...
            //Now we need to iterate through the adjacent supervoxels and make a point cloud of them
            PointCloudT adjacent_supervoxel_centers;
            std::multimap<uint32_t,uint32_t>::iterator adjacent_itr = adjacency->equal_range (supervoxel_label).first;
            for ( ; adjacent_itr!=adjacency->equal_range (supervoxel_label).second; ++adjacent_itr)
            {
                pcl::Supervoxel<PointT>::Ptr neighbor_supervoxel = voxels->at (adjacent_itr->second);
                adjacent_supervoxel_centers.push_back (neighbor_supervoxel->centroid_);
